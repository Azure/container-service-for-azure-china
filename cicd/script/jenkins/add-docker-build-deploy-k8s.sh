...
credentials_desc="Private Docker Container Registry Credentials"
job_short_name="basic-docker-build-deploy-to-K8s"
job_display_name="Basic Docker Build Deploy to Kubernetes"
job_description="A basic pipeline that builds a Docker container and deploy it to Kubernetes cluster. The job expects a Dockerfile at the root of the git repository"
repository="${USER}/myfirstapp"
scm_poll_schedule=""
scm_poll_ignore_commit_hooks="0"
artifacts_location="https://raw.githubusercontent.com/Azure/devops-sample-solution-for-azure-china/eshop/cicd/script"
...
job_xml=${job_xml//'{insert-groovy-script}'/"$(curl -s ${artifacts_location}/jenkins/basic-docker-build-deploy-k8s.groovy${artifacts_location_sas_token})"}
